#include <rclcpp/rclcpp.hpp>
#include <sensor_msgs/msg/imu.hpp>
#include <sensor_msgs/msg/laser_scan.hpp>
#include <nav_msgs/msg/odometry.hpp>
#include <Eigen/Dense>
#include <cmath>

class EKFNode : public rclcpp::Node {
public:
    EKFNode() : Node("ekf_node") {
        // Subscribers for IMU and LiDAR
        imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
            "/imu_plugin/out", 10, std::bind(&EKFNode::imuCallback, this, std::placeholders::_1));
        lidar_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
            "/scan", 10, std::bind(&EKFNode::lidarCallback, this, std::placeholders::_1));
        
        // Publisher for the estimated odometry
        odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("/odom_ekf", 10);

        // Initial state: [x, y, vx, vy, yaw]
        state_ = Eigen::VectorXd::Zero(5);
        // Initial covariance matrix
        covariance_ = Eigen::MatrixXd::Identity(5, 5);
        // Initial IMU bias for linear acceleration: [bx, by]
        imu_bias_ = Eigen::VectorXd::Zero(2);
    }

private:
    // Subscribers and publisher
    rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr imu_sub_;
    rclcpp::Subscription<sensor_msgs::msg::LaserScan>::SharedPtr lidar_sub_;
    rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr odom_pub_;

    // State vector and covariance matrix
    Eigen::VectorXd state_;  // [x, y, vx, vy, yaw]
    Eigen::MatrixXd covariance_;  // State covariance matrix
    Eigen::VectorXd imu_bias_;  // IMU bias for linear acceleration

    // Parameters for EKF
    double dt_ = 0.1;  // Time step
    double process_noise_ = 0.1;  // Process noise covariance
    double measurement_noise_ = 0.01;  // Measurement noise covariance
    double bias_correction_rate_ = 0.001;  // IMU bias correction rate

    void imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
        // Extract IMU data
        double yaw_rate = msg->angular_velocity.z;
        double ax = msg->linear_acceleration.x - imu_bias_(0);  // Corrected IMU data
        double ay = msg->linear_acceleration.y - imu_bias_(1);

        // Bias correction for IMU drift
        imu_bias_(0) += bias_correction_rate_ * ax;
        imu_bias_(1) += bias_correction_rate_ * ay;

        // Predict step of the EKF
        predict(ax, ay, yaw_rate, dt_);
    }

    void lidarCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
        std::vector<Eigen::Vector2d> lidar_points;

        // Extract valid LiDAR points
        for (size_t i = 0; i < msg->ranges.size(); i++) {
            if (msg->ranges[i] >= msg->range_min && msg->ranges[i] <= msg->range_max) {
                double angle = msg->angle_min + i * msg->angle_increment;
                double lidar_x = msg->ranges[i] * std::cos(angle);
                double lidar_y = msg->ranges[i] * std::sin(angle);
                lidar_points.emplace_back(lidar_x, lidar_y);
            }
        }

        // Use LiDAR data for the update step if there are valid points
        if (!lidar_points.empty()) {
            Eigen::Vector2d avg_measurement = Eigen::Vector2d::Zero();
            for (const auto& point : lidar_points) {
                avg_measurement += point;
            }
            avg_measurement /= lidar_points.size();  // Calculate average LiDAR point

            // Update step of the EKF with the average LiDAR measurement
            update(avg_measurement);
        }
    }

    void predict(double ax, double ay, double yaw_rate, double dt) {
        // State transition update
        state_(2) += ax * dt;  // vx
        state_(3) += ay * dt;  // vy
        state_(0) += state_(2) * dt;  // x
        state_(1) += state_(3) * dt;  // y
        state_(4) += yaw_rate * dt;  // yaw

        // State transition matrix F
        Eigen::MatrixXd F = Eigen::MatrixXd::Identity(5, 5);
        F(0, 2) = dt;
        F(1, 3) = dt;

        // Update the covariance matrix
        covariance_ = F * covariance_ * F.transpose() + process_noise_ * Eigen::MatrixXd::Identity(5, 5);
    }

    void update(const Eigen::Vector2d& measurement) {
        // Measurement matrix H
        Eigen::MatrixXd H = Eigen::MatrixXd::Zero(2, 5);
        H(0, 0) = 1;  // For x
        H(1, 1) = 1;  // For y

        // Predicted measurement
        Eigen::VectorXd predicted_measurement = H * state_;
        // Measurement innovation (residual)
        Eigen::Vector2d innovation = measurement - predicted_measurement;

        // Innovation covariance
        Eigen::MatrixXd S = H * covariance_ * H.transpose() + measurement_noise_ * Eigen::MatrixXd::Identity(2, 2);
        // Kalman gain
        Eigen::MatrixXd K = covariance_ * H.transpose() * S.inverse();

        // Update state with measurement
        state_ += K * innovation;
        // Update covariance matrix
        covariance_ = (Eigen::MatrixXd::Identity(5, 5) - K * H) * covariance_;

        // Publish the updated state as odometry
        publishOdometry();
    }

    void publishOdometry() {
        auto odom_msg = std::make_shared<nav_msgs::msg::Odometry>();
        odom_msg->header.stamp = this->now();
        odom_msg->header.frame_id = "odom";
        odom_msg->child_frame_id = "base_link";
        
        // Set position and orientation
        odom_msg->pose.pose.position.x = state_(0);
        odom_msg->pose.pose.position.y = state_(1);
        odom_msg->pose.pose.position.z = 0.0;

        double yaw = state_(4);
        odom_msg->pose.pose.orientation.x = 0.0;
        odom_msg->pose.pose.orientation.y = 0.0;
        odom_msg->pose.pose.orientation.z = std::sin(yaw / 2.0);
        odom_msg->pose.pose.orientation.w = std::cos(yaw / 2.0);

        // Set linear and angular velocity
        odom_msg->twist.twist.linear.x = state_(2);
        odom_msg->twist.twist.linear.y = state_(3);
        odom_msg->twist.twist.angular.z = yaw;

        // Publish odometry message
        odom_pub_->publish(*odom_msg);
    }
};

int main(int argc, char** argv) {
    rclcpp::init(argc, argv);
    auto ekf_node = std::make_shared<EKFNode>();
    rclcpp::spin(ekf_node);
    rclcpp::shutdown();
    return 0;
}
